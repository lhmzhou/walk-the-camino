...
var generateReport = function (options) {
    ...
    console.log("feature output :"+ featureOutput)
    ...
    var setStats = function (suite) {
        ...
        var basedirectory = hierarchyReporter.getBaseDir(suite);

        featureOutput.forEach(function (feature) {
            feature.hierarchy = hierarchyReporter.getFeatureHierarchy(feature.uri, basedirectory);
            feature.scenarios = {};
                            ...
                            if (embeddingType === 'text/plain') {
                                ...
                                if ((options.storeScreenshots && options.storeScreenshots === true) ||
                                    ...
                                    if (!fs.existsSync(screenshotsDirectory)) {
                                        fs.mkdirectorySync(screenshotsDirectory);
                                    }
...
function generate(options, callback) {
    function isValidJsonFile() {
        //removeFlatTxtFromJson(options);
        console.log("This is entry point ");
        options.jsonFile = options["directory"]+"report/in/report.json";
...
function removeFlatTxtFromJson(options) {
    var fs  = require("fs");
    fs.unlink(options['directory']+'report/in/output.json',function(err){
        if(err) return console.log(err);
        console.log('file deleted successfully');
    });
    fs.readFileSync(options.jsonFile).toString().split('\n').forEach(function (line) {
        //console.log(line);
        if (line.includes("testing:") || line.includes("PASS") || line.includes("exit status 1") || line.includes("FAIL")) {
            line = " "
        }fs.delete
        fs.appendFileSync(options['directory']+"report/in/output.json", line.toString() + "\n");
    });
}
